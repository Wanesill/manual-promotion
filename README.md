...
- **Цикл одного аккаунта.** Воркер бесконечно повторяет итерации с целевым
  ...
  ~4 ч — это норма. Soft-кэп `MAX_ITERATION_S=6 ч` проверяется между ad'ами:
  если затянулись — выходим из прохода, в следующей итерации стартуем
  заново (свежая загрузка, новый токен если истёк). Чтобы бюджет/метрики
  не сильно устаревали, каждые 5 мин внутри итерации воркер дозагружает
  `stats` и пересчитывает `now`.
...
- **Токены аккаунтов в БД не обновляем.** Refresh устаревшего
  `Account.access_token` — задача родительского сервиса. В начале каждой
  итерации `_resolve_avito(account)` строит `AvitoService`: если
  `access_token` есть и до `expires_in` остался запас ≥ `TOKEN_REFRESH_THRESHOLD_S`
  (1 ч) — используем его; иначе дёргаем `AvitoService.authenticate(client_id,
  client_secret)` и берём токен оттуда. Результат **не пишем в БД** и не
  кэшируем между итерациями. Если нет credentials — `LOG_DISABLED_BY_TOKEN_EXPIRED`;
  если `authenticate` упал/вернул мусор — `LOG_DISABLED_BY_AUTH_FAILED`.
...
- **Все user-visible строки на русском.** Константы `LOG_*` и текст заметок
  — часть публичного контракта с родительским API (сопоставление по точному
  совпадению строки) — переводить нельзя.
...
`MAX_ITERATION_S=21600` = 6 ч) зашиты константами в
[`app/dispatcher/process_dispatcher.py`](./app/dispatcher/process_dispatcher.py)
