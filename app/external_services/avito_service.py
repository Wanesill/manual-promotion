...
Аутентификация (`POST /token` через `authenticate`) дёргается caller'ом
(`process_dispatcher._resolve_avito`) когда `Account.access_token` в БД
отсутствует/протухает в ближайший час. Результат в БД мы не пишем —
живёт только в локальной `AvitoService`-инстансе на текущую итерацию.
...
class AvitoService:
    ...
    async def authenticate(
        ...
        Используется в `process_dispatcher._resolve_avito` когда в БД нет
        валидного `Account.access_token`. Результат **в БД не сохраняем**
        — токен живёт только в локальной `AvitoService`-инстансе на
        текущую итерацию воркера. Refresh устаревшего токена в БД —
        задача родительского API.
